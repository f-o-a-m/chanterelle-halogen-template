...
simpleStorageSpec
  :: forall e r.
     TestConfig ( simpleStorage :: SimpleStorageReceipt
                | r
                )
  -> Spec ( eth     :: ETH
          , avar    :: AVAR
...
simpleStorageSpec testConfig@{provider, accounts, simpleStorage} = do
  let
    ...
    ssTxOpts = defaultTransactionOptions # _to ?~ simpleStorage.address
                                         # _from ?~ primaryAccount

  describe "Should test the deployment of SimpleStorage" $ do
    ...
    it "Can update the count and monitor for even numbers" $ do
      ...
      let
        ...
        countMonitor countVar = \(SimpleStorage.CountSet {newCount}) -> do
          ...
          if isEvenBN $ unUIntN newCount
             then liftAff (putVar newCount countVar) *> pure TerminateEvent
             else pure ContinueEvent

      -- Run the count monitor asynchronously
      countFiber <- forkWeb3 provider $ do
